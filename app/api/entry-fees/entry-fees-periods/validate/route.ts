...
export async function POST(req: NextRequest) {
  try {
    ...
    await applyEntryFeesPeriodBatch(parsed.data, { dryRun: true });
    return withCors(NextResponse.json({ ok: true, errors: [] }));
  } catch (err: any) {
    ...
    if (isPgOverlapError(err)) {
      return withCors(
        NextResponse.json(
          { ok: false, code: 'PERIOD_OVERLAP', message: error.message, errors: [error] },
          { status: 409 },
        ),
      );
    }
    ...
    if (isPeriodNotFoundError(err)) {
      return withCors(
        NextResponse.json(
          { ok: false, code: 'PERIOD_NOT_FOUND', message: error.message, errors: [error] },
          { status: 404 },
        ),
      );
    }
